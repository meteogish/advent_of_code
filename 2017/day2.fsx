...
let convertToNumbersMatrix (s: string) =
    s.Split(System.Environment.NewLine)
    |> Seq.map (fun line -> line.Split('\t') |> Seq.map int )

let numbers = convertToNumbersMatrix input

let answerPartOne =
    numbers
    |> Seq.sumBy (fun line -> (line |> Seq.max) - (line |> Seq.min))
        ...
        (fun line ->
            ...
            |> Seq.choose 
                (fun (first, second) ->
                    ...
                    | (max, min) when max > min && max % min = 0 -> max / min |> Some 
                    | (min, max) when max > min && max % min = 0 -> max / min |> Some 
                    | _ -> None)
            |> Seq.head
        )
